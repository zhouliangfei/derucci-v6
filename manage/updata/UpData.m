...
- (void)viewDidUnload
{
    ...
    //
    [appURL release],appURL=nil;
    
    [appVersion release],appVersion=nil;
    
    [ftpConfig release],ftpConfig=nil;
    
    [versionKey release],versionKey=nil;
    
    [versionValue release],versionValue=nil;
    ...
    [dataTable release],dataTable=nil;
    
    [appTable release],appTable=nil;
...
-(void)getDataVersionList:(NSMutableDictionary**)files
{
    ...
    //查询是否有更新
    id webdata = [Utils getURL:[REMOTEURL stringByAppendingFormat:UPDATAVERSION,[NSNumber numberWithInt:lastVersion - 1]]];
            ...
            for (NSDictionary *tmp in archives)
            {
                ...
                for (int i=0;i<[paths count];i++)
                {
                    if (version > lastVersion || (version == lastVersion && index > lastIndex)) 
                    {
                        ...
                        NSString *save = [Utils pathForDocument:[[val objectForKey:@"filePath"] lastPathComponent]];
...
- (void)appCellTouch:(UpdataCell*)sender
{
    ...
    if (url)
    {
        [[NSUserDefaults standardUserDefaults] setObject:[[NSDate date] stringFromFormatter:@"yyyy-MM-dd"] forKey:CURSYSTEMDATE];
...
-(void)writeQueueAttribute
{
    ...
    if (queue) 
    {
        NSString *date = [[NSDate date] stringFromFormatter:@"yyyy-MM-dd"];
